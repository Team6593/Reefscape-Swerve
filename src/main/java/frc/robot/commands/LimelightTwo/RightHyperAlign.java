...
package frc.robot.commands.LimelightTwo;
...
public class RightHyperAlign extends Command {
  ...
  public RightHyperAlign(boolean isRightScore, CommandSwerveDrivetrain drivebase, 
  double maxDtSpeed, double maxAngularRate, SwerveRequest.RobotCentric robotCentric) {
    ...
    rotController = new PIDController(.008, 0, 0);  // Rotation
  ...
  public void initialize() {
    ...
    if(LimelightTwo.autoEstimateDistance() > 25) {
      rotController.setP(0.041);
    } else if(LimelightTwo.autoEstimateDistance() < 25) {
      rotController.setP(0.041);
    } else if(LimelightTwo.autoEstimateDistance() <= 18) {
  ...
  public void execute() {
    if (LimelightHelpers.getTV("limelight-two") && LimelightHelpers.getFiducialID("limelight-two") == tagID) {
      ...
      drivebase.setControl(
        robotCentric
          .withVelocityX(xSpeed * (maxDtSpeed * .5)) // forward backward
